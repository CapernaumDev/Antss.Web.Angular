    ...
    () =>
      this.actions$.pipe(
        ...
        tap(([action, afterLoginRedirect, currentUser]) => {
          if (action.loginResult.accessToken) localStorage.setItem('access-token', action.loginResult.accessToken);
      ...
      this.actions$.pipe(
        ...
        tap(([action, previousUrl]) => {
          this.router.navigateByUrl(
            previousUrl == null || previousUrl == '/' || previousUrl == '/login' ? action.defaultRedirect : previousUrl
          );
        })
      ),
    { dispatch: false }
